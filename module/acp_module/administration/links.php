...
+--------------------------------------------------------*/
	...
	if (isset($_GET['action']) && $_GET['action'] == "refresh")
		{
			...
			while ($data = db_array($result))
				{
					$result2 = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET link_order='$i' WHERE link_id='".$data['link_id']."'");
					$i++;
				}
			redirect(WCF_SELF);
		}
	elseif ((isset($_GET['action']) && $_GET['action'] == "moveup") && (isset($_GET['link_id']) && isnum($_GET['link_id'])))
		{
			selectdb("wcf");
			$data = db_assoc(db_query("SELECT `link_id` FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_order`='".intval($_GET['order'])."'"));
			$result = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET `link_order`=link_order+1 WHERE `link_id`='".$data['link_id']."'");
			$result = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET `link_order`=link_order-1 WHERE `link_id`='".$_GET['link_id']."'");
			redirect(WCF_SELF);
		}
	...
	elseif ((isset($_GET['action']) && $_GET['action'] == "delete") && (isset($_GET['link_id']) && isnum($_GET['link_id'])))
		{
			selectdb("wcf");
			$data = db_assoc(db_query("SELECT `link_order` FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_id`='".$_GET['link_id']."'"));
			$result = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET `link_order`=link_order-1 WHERE `link_order`>'".$data['link_order']."'");
			$result = db_query("DELETE FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_id`='".$_GET['link_id']."'");
	...
	else
		{
						...
						if ($link_name && $link_url)
							{
								if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['link_id']) && isnum($_GET['link_id'])))
									{
										$old_link_order = db_result(db_query("SELECT `link_order` FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_id`='".$_GET['link_id']."'"), 0);
		
										if ($link_order > $old_link_order)
											{
												$result = dbquery("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET `link_order`=link_order-1 WHERE `link_order`>'$old_link_order' AND `link_order`<='$link_order'");
											}
										...
										$result = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET `link_name`='$link_name', `link_url`='$link_url', `link_visibility`='$link_visibility', `link_position`='$link_position', `link_order`='$link_order' WHERE `link_id`='".$_GET['link_id']."'");
										redirect(WCF_SELF."?status=su");
									}
								else
									{
										if (!$link_order) { $link_order = db_result(db_query("SELECT MAX(`link_order`) FROM ".DB_ACP_NAVIGATION_LINKS.""), 0) + 1; }
		
										$result = db_query("UPDATE ".DB_ACP_NAVIGATION_LINKS." SET link_order=link_order+1 WHERE link_order>='$link_order'");
										$result = db_query("INSERT INTO ".DB_ACP_NAVIGATION_LINKS." (`link_name`, `link_url`, `link_visibility`, `link_position`, `link_order`) VALUES ('".$link_name."', '".$link_url."', '".$link_visibility."', '".$link_position."', '".$link_order."')");
			...
			if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['link_id']) && isnum($_GET['link_id'])))
					{
						selectdb("wcf");
						$result = db_query("SELECT * FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_id`='".$_GET['link_id']."'");
			...
			selectdb("wcf");
			$result = db_query("SELECT * FROM ".DB_ACP_NAVIGATION_LINKS." WHERE `link_position`='1' OR `link_position`='2' ORDER BY `link_order`");
			if (db_num_rows($result))
				{
					$i = 0; $k = 1;
							...
							if ($data['link_name'] != "---" && $data['link_url'] == "---")
							...
							else
								{
									if (strstr($data['link_url'], "http://") || strstr($data['link_url'], "https://"))
										...
										{
											echo"<a href='".$modules['acp_module'].$data['link_url']."'>".$data['link_name']."</a>";
										}
								}
							echo"</td>";
