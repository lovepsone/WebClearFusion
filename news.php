...
+--------------------------------------------------------*/
	...
	require_once THEMES."templates/header.php";
  	...
  	$result = WCF::$DB->db_query("SELECT count(`news_date`) as number FROM ".DB_NEWS);
	$kolzap = WCF::$DB->db_array($result);

	if ($kolzap['number'] > WCF::$settings['page_news'])
	{
    		$page_len = WCF::$settings['page_news'];
		...
		{
			$start_rec = ((int)$_GET['page']-1)*WCF::$settings['page_news'];
		}
	}
	else
	{
  	...
  	$result = WCF::$DB->db_query("SELECT * FROM ".DB_NEWS." LEFT JOIN ".DB_NEWS_CATS." ON `news_cat_id`=`news_cat` LEFT JOIN ".DB_USERS." ON ".DB_USERS.".`user_id`=".DB_NEWS.".`news_author` ORDER BY `news_date` DESC limit ".$start_rec.",".$page_len);
  	...
  	if (WCF::$DB->db_num_rows($result))
	{
     		while ($data = WCF::$DB->db_array($result))
		{
  		...
  		if ($kolzap['number'] > WCF::$settings['page_news'])
		{
  			$PageCounter = ceil($kolzap['number'] / WCF::$settings['page_news']);
