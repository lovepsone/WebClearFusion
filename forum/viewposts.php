	...
	if (isset($_GET['thread_id']) AND isset($_GET['forum_id']))
		{
			...
			$thread_id = addslashes($_GET["thread_id"]);
  			...
  			$result = WCF::$DB->db_query("SELECT count(`post_date`) as kol FROM ".DB_FORUMS_POSTS." WHERE `forum_id`='$forum_id' AND `thread_id`='$thread_id'");
			$p_kolzap = WCF::$DB->db_array($result);

			if ($p_kolzap['kol'] > WCF::$settings['page_forum_posts'])
				{
    					$page_len_p = WCF::$settings['page_forum_posts'];
						...
						{
							$start_rec_p = ((int)$_GET['page']-1)*WCF::$settings['page_forum_posts'];
						}
				}
			else
				{
			...
			$name_thread = WCF::$DB->db_query($result);
			$result = WCF::$DB->db_query($result);
			opentable();
			//==============================
			// Функция добавления просмотров
			if ($result != 0)
				{
					WCF::$DB->db_query("UPDATE ".DB_FORUMS_THREADS." SET `thread_views`=thread_views+1 WHERE `thread_id`='".$thread_id."'");
				}

			if (WCF::$DB->db_num_rows($result) > 0 )
				{
					$name_thread = WCF::$DB->db_aassoc($name_thread);

					echo"<tr><th width='100%' colspan='2' class='tbl'>".$name_thread['thread_subject']."</th></tr>";

					while ($data = WCF::$DB->db_array($result))
						{
 							$result_user = WCF::$DB->db_query("SELECT * FROM ".DB_USERS." WHERE `user_id`=".$data['user_id']." LIMIT 1");
  							$usr = WCF::$DB->db_aassoc($result_user);
  					...
  					if ($p_kolzap['kol'] > $config['page_forum_posts'])
						{
  							$page_counter_p = ceil($p_kolzap['kol'] / WCF::$settings['page_forum_posts']);
			...
			if (isset($_SESSION['user_id']) || ($_SESSION['ip'] == $_SERVER['REMOTE_ADDR']))
				{
					if($_POST['posts'])
						{
							// Создаем сообщение
							WCF::$DB->db_query("INSERT INTO ".DB_FORUMS_POSTS."
